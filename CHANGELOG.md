...
- MessageCard & fetchMessage methods@`StreamTalkMessenger.php` fallback.

## v1.5.1 (2022-06-09)

### Fixed

- Sync the `sending a message form`'s allowed files/images with the `config` file (Update sendForm.blade.php [#190](https://github.com/munafio/StreamTalk/pull/190))
...
- [Gravatar](https:://gravatar.com) support (optional, can be changed at config/StreamTalk.php).
- Delete Message by ID.
- Laravel's Storage disk now supported and can be changed from the config.

### Changed

- File upload (user avatar & attachments) `allowed files` and `max size` now can be changed from one place which is (config/StreamTalk.php).
